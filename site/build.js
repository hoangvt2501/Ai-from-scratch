...
// Fork-aware: set AIFS_REPO / AIFS_SITE_ORIGIN in the deploy environment so a
// fork's site links to (and fetches lesson docs from) its own repo.
const REPO_SLUG = process.env.AIFS_REPO || 'rohitg00/ai-engineering-from-scratch';
const GITHUB_BASE = 'https://github.com/' + REPO_SLUG + '/tree/main/';
const SITE_ORIGIN = (process.env.AIFS_SITE_ORIGIN || 'https://aiengineeringfromscratch.com').replace(/\/+$/, '');
...
function writeBuildMeta() {
  let ref = process.env.VERCEL_GIT_COMMIT_REF || process.env.GITHUB_REF_NAME || '';
  if (!ref) {
    try {
  ...
  const js = '// Auto-generated by build.js on each deploy — do not edit.\n'
    ...
    + 'window.__AIFS_REPO = ' + JSON.stringify(REPO_SLUG) + ';\n';
  fs.writeFileSync(path.join(__dirname, 'build-meta.js'), js, 'utf8');
  console.log('   wrote build-meta.js (repo: ' + REPO_SLUG + ', ref: ' + ref + ')');
}
